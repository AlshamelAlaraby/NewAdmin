...
export default {
  page: {
    title: "Menu tree",
    meta: [{ name: "description", content: "Menu tree" }],
  },
  components: {
    Layout,
    PageHeader,
  ...
  methods: {
    setChildNodes(result) {
      adminApi.get(`/menu-tree/child-nodes/${result.node.id}`).then((res) => {
        this.isLoader = false;
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/menu-tree?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
    ...
    getDataCurrentPage() {
      if (
        ...
        adminApi
          .get(
            `/menu-tree?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}`
          )
          .then((res) => {
            let l = res.data;
            this.menuTrees = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/menu-tree/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
     ...
     */
    resetModalHidden() {
      this.create = { name: "", name_e: "", parent_id: null, menu_id: null, is_active: "active" };
      this.$nextTick(() => {
        this.$v.$reset();
      });
      this.errors = {};
    ...
    AddSubmit() {
      ...
      if (this.$v.create.$invalid) {
        ...
        adminApi
          .post(`/menu-tree`, this.create)
          .then((res) => {
            this.getData();
            this.is_disabled = true;
            this.getRootNodes(this.create.menu_id);
            setTimeout(() => {
              Swal.fire({
                icon: "success",
    ...
    async getRootNodes(id) {
      ...
      await adminApi
        .get(`/menu-tree/menu-root-nodes/${id}`)
        .then((res) => {
    ...
    async resetModalEdit(id) {
      ...
      this.edit.menu_id = module.Menu.id;
      await this.getRootNodes(this.edit.menu_id);
      this.errors = {};
      this.current_id = id;
    },
    /**
        ...
        <div class="card">
          <div class="card-body">
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.menuTree") }}</h4>
              ...
              <div class="col-xs-10 col-md-9 col-lg-7 d-flex align-items-center justify-content-end">
                <div>
                  ...
                  <b-dropdown variant="primary" :html="`${$t('general.setting')} <i class='fe-settings'></i>`"
                    ...
                    <b-form-checkbox v-model="setting.menu_id" class="mb-1">
                      {{ $t("general.menu") }}
                    </b-form-checkbox>
                  ...
                  <div class="d-inline-flex align-items-center pagination-custom">
                    <div class="d-inline-block" style="font-size: 15px">
                      {{ menuTreesPagination.from }}-{{ menuTreesPagination.to }} /
                      {{ menuTreesPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a href="javascript:void(0)" :style="{
                        'pointer-events':
                          menuTreesPagination.current_page == 1 ? 'none' : '',
                      }" @click.prevent="getData(menuTreesPagination.current_page - 1)">
                        <span>&lt;</span>
                      </a>
                      ...
                      <a href="javascript:void(0)" :style="{
                        'pointer-events':
                          menuTreesPagination.last_page == menuTreesPagination.current_page
                            ? 'none'
                            : '',
                      }" @click.prevent="getData(menuTreesPagination.current_page + 1)">
                        <span>&gt;</span>
            ...
            <!--  create   -->
            <b-modal id="create" :title="$t('general.menu')" title-class="font-18" body-class="p-4 " :hide-footer="true"
              size="lg" @show="resetModal" @hidden="resetModalHidden">
              <form>
                ...
                <div class="row">
                  <div class="col-8">
                    <TreeBrowser @deleteClicked="deleteModule($event.id, true, create.menu_id)"
                      :currentNodeId="create.parent_id" @onClick="setCreateCurrentNode" @nodeExpanded="setChildNodes"
                      :nodes="rootNodes" />
                    ...
                    <div class="row">
                      <div class="col-md-12 position-relative">
                        <div class="form-group">
                          ...
                          <multiselect @input="getRootNodes" v-model="create.menu_id"
                            :options="menus.map((type) => type.id)" :custom-label="
                              (opt) =>
                                $i18n.locale == 'ar'
                                  ? menus.find((x) => x.id == opt).name
                                  : menus.find((x) => x.id == opt).name_e
                            " :class="{
  'is-invalid': $v.create.menu_id.$error || errors.menu_id,
}">
                          </multiselect>
                          <div v-if="!$v.create.menu_id.required" class="invalid-feedback">
                            {{ $t("general.fieldIsRequired") }}
                          </div>

                          <template v-if="errors.menu_id">
                            <ErrorMessage v-for="(errorMessage, index) in errors.menu_id" :key="index">{{ errorMessage
                            }}</ErrorMessage>
                          </template>
                        </div>
                      </div>
                      <div class="col-12 direction" dir="rtl">
                        <div class="form-group">
                          ...
                          <template v-if="errors.name">
                            <ErrorMessage v-for="(errorMessage, index) in errors.name" :key="index">{{ $t(errorMessage) }}
                            </ErrorMessage>
                          </template>
                        </div>
                      </div>
                      <div class="col-12 direction-ltr" dir="ltr">
                        <div class="form-group">
                          ...
                          <template v-if="errors.name_e">
                            <ErrorMessage v-for="(errorMessage, index) in errors.name_e" :key="index">{{ $t(errorMessage)
                            }}</ErrorMessage>
                          </template>
                        </div>
                      </div>
            ...
            <div class="table-responsive mb-3 custom-table-theme position-relative">
              ...
              <table class="table table-borderless table-hover table-centered m-0" ref="exportable_table" id="printMe">
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="menuTrees.sort(sortString('name'))"></i>
                          <i class="fas fa-arrow-down" @click="menuTrees.sort(sortString('-name'))"></i>
                        </div>
                      </div>
                    </th>
                    <th v-if="setting.name_e">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="menuTrees.sort(sortString('name_e'))"></i>
                          <i class="fas fa-arrow-down" @click="menuTrees.sort(sortString('-name_e'))"></i>
                        </div>
                      </div>
                    </th>
                ...
                <tbody v-if="menuTrees.length > 0">
                  <tr @click.capture="checkRow(data.id)" @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in menuTrees" :key="data.id" class="body-tr-custom">
                    ...
                    <td v-if="setting.menu_id">
                      <template v-if="data.Menu">
                        {{ $i18n.locale == "ar" ? data.Menu.name : data.Menu.name_e }}
                      </template>
                    </td>
                    <td v-if="enabled3" class="do-not-print">
                      <div class="btn-group">
                      ...
                      <!--  edit   -->
                      <b-modal :id="`modal-edit-${data.id}`" :title="$t('general.menu')" title-class="font-18"
                        body-class="p-4" size="lg" :ref="`edit-${data.id}`" :hide-footer="true"
                        ...
                        <form>
                          ...
                          <div class="row">
                            <div class="col-8">
                              <TreeBrowser @deleteClicked="deleteModule($event.id, true, edit.menu_id)"
                                :currentNodeId="edit.parent_id" @onClick="setUpdateCurrentNode"
                            ...
                            <div class="col-4">
                              <div class="row">
                                <div class="col-md-12 position-relative">
                                  <div class="form-group">
                                    <label class="my-1 mr-2">{{ $t("general.menu") }}</label>
                                    <multiselect @input="getRootNodes" v-model="edit.menu_id"
                                      :options="menus.map((type) => type.id)" :custom-label="
                                        (opt) =>
                                          $i18n.locale == 'ar'
                                            ? menus.find((x) => x.id == opt).name
                                            : menus.find((x) => x.id == opt).name_e
                                      " :class="{
  'is-invalid': $v.edit.menu_id.$error || errors.menu_id,
}">
                                    </multiselect>
                                    <div v-if="!$v.edit.menu_id.required" class="invalid-feedback">
                                      {{ $t("general.fieldIsRequired") }}
                                    </div>

                                    <template v-if="errors.menu_id">
                                      <ErrorMessage v-for="(errorMessage, index) in errors.menu_id" :key="index">{{
                                        errorMessage
                                      }}</ErrorMessage>
                                    </template>
                                  </div>
                                ...
                                <div class="col-12 direction" dir="rtl">
                                  <div class="form-group">
                                    ...
                                    <input type="text" class="form-control arabicInput" v-model="$v.edit.name.$model"
                                      ...
                                      }" :placeholder="$t('general.Name')" @keyup="arabicValue(edit.name)"
                                      id="field-u-1" />
                                ...
                                <div class="col-12 direction-ltr" dir="ltr">
                                  <div class="form-group">
                                    ...
                                    <input type="text" class="form-control englishInput" v-model="$v.edit.name_e.$model"
                                      ...
                                      }" :placeholder="$t('general.Name_en')" @keyup="englishValue(edit.name_e)"
                                      id="field-u-2" />
