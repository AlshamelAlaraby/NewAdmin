<template>
    <div>
        ...
        <b-modal id="create_or_edit"
            :title="`${action == 'create' ? $t('general.Add Program') : $t('general.Update') + ' ( ' + edit_node.name + ')'}`"
                    ...
                    <div class="col-md-12">
                        <template>
                            <div class="row">
                                ...
                                <div class="col-md-12" v-if="action == 'edit' && edit_node.type != 'program'">
                                    <label class="my-1 mr-2">{{
                                        $t("general.Dashboard")
                                    }}</label>
                                    <multiselect @search-change="asyncFindDashboard"
                                        v-model="create_or_edit.module_dashboard_id"
                                        :options="dashboards_limit.map((type) => type.id)"
                                        :custom-label="customLabelFunction" :class="{
                                            'is-invalid':
                                                $v.create_or_edit.module_dashboard_id.$error ||
                                                errors.module_dashboard_id,
                                        }">
                                    </multiselect>
                                    <div v-if="!$v.create_or_edit.module_dashboard_id.required" class="invalid-feedback">
                                    ...
                                    <template v-if="errors.module_dashboard_id">
                                        <ErrorMessage v-for="(
                                        errorMessage, index
                                      ) in errors.module_dashboard_id" :key="index">{{ errorMessage }}</ErrorMessage>
                                    </template>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div style="height:200px;overflow-y:scroll">
                                            <div class="d-flex justify-content-between mb-3">
                                                <input type="text" class='form-control' v-model="filter_icon_input"
                                                    @keyup="filter_icons" :placeholder="$t('general.Filter Icons')">
                                                <a class="btn btn-danger mx-1" v-show="create_or_edit.icon"
                                                    @click.prevent="create_or_edit.icon = ''"><i
                                                        class="far fa-times-circle"></i></a>
                                            </div>
                                            <i v-for="icon in icons"
                                                :class="[icon, 'mx-1', create_or_edit.icon == icon ? 'text-info font-weight-bold' : '']"
                                                :key="icon" @click.prevent="create_or_edit.icon = icon"
...
export default {
    ...
    validations: {
        create_or_edit: {
            ...
            sort: {
                required(value) {
                    let check_sort = this.action == 'edit' && this.edit_node.type != 'program';
            ...
            module_dashboard_id: {
                required(value) {
                    let check_module_dashboard_id = this.action == 'edit' && this.edit_node.type != 'program';
            ...
            if (this.$v.create_or_edit.$invalid) {
                ...
                adminApi
                    ...
                    .then((res) => {
                        ...
                        this.$emit('add_new_program', res.data)
        ...
        editSubmit() {
            ...
            if (this.$v.create_or_edit.$invalid) {
                ...
                adminApi
                    ...
                    .then((res) => {
                        this.$bvModal.hide(`create_or_edit`);
                        this.$emit('updated_program_or_module', { program_id: this.edit_node.type == 'module' ? res.data.parent_id : res.data.id, type: this.edit_node.type, program: res.data })
