  ...
  <draggable class="dragArea" tag="div" :list="nodes">
    ...
    >
      <span class="type" @click="nodeClicked(node)"
        ><i
          v-if="node.haveChildren || node.screen_tree"
          :class="isExpanded(node) ? 'fas fa-minus' : 'fas fa-plus'"
        ></i
      ></span>
      <span
        :class="{
          active: secondNodeNotChoosed && depth==1?false:currentNodeId == node.id}"
        class="title-tree"
      ...
      <span v-if="editable" @click="$emit('editClicked', node)">
        <i
          style="background-color: unset; position: relative; bottom: 1px"
          class="fas fa-edit text-success"
        ></i>
      </span>
      <span
        @click="$emit('deleteClicked', node)"
        v-if="!node.haveChildren && !node.isScreen && !node.screen_tree"
      >
        <i class="fas fa-minus delete text-danger"></i>
      </span>
      <TreeBrowser
      :secondNodeNotChoosed="secondNodeNotChoosed"
        :editable="editable"
